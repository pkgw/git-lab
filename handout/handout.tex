...
In \git, a branch is just a name referring to a commit. That's all you need to
reconstruct the entire project history back to its inception: the named commit
points to its parent, which points to its parent, and so on. The following
command prints out the names of the branches in your repository:
...
the branch is updated to point to that new commit. That's all you need to do
to move the branch forward, and by induction that's still all we need to
...
\typeone{cat .git/refs/heads/master}{Manually print the master branch commit
  id.}
...
Really, each commit has \i{one or more parents}.
