...
\begin{typeme}
git branch conflict origin/master & New branch with pristine files. \\
git checkout conflict & Switch to \t{conflict} branch. \\
...
\fillme{1.5in}{What does the commit history of the \t{conflict} branch look
  like, as a graph?}
...
\section{git remote, git fetch, git pull, git push}
...
known as \i{remotes}. Every cloned repository starts with a remote called
\s{origin}, which you've probably been seeing mentioned by various tools over
the course of the lab. You can learn about your remotes with, unsurprisingly,
the \cmd{remote} command.
...
that you're working on (\s{master}), and you've got one with commits that you
want to import (\s{origin/master} --- note that the \s{remotes/} prefix isn't
necessary). That sounds like time for a \cmd{merge}!

\begin{typeme}
git checkout conflict & Make sure we're on the right branch. \\
